...
| **Export objects** | writes the selection to `.plc-framework\exports\`, in a tree shaped like the project's — so two exports can be compared. Blocks, technology objects, tag tables and PLC data types as `.xml`; a PLC's alarm text lists as one `.xlsx`, which is all Openness offers for them |
