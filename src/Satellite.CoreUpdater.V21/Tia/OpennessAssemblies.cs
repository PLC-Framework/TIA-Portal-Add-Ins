...
namespace Satellite.CoreUpdater.Tia
{
    ...
    internal static class OpennessAssemblies
    {
        ...
        /// Preferred when a station has several installed, because a V21 build must not bind
        /// to V20's assemblies - different public key tokens, so the load would fail anyway,
        /// just later and with a worse message.
        /// </summary>
        private const string Version = "21.0";

        /// <summary>The same version as a path segment: `…\Portal V21\PublicAPI\V21\`.</summary>
        private const string Segment = "V21";
        ...
        private static List<string> Folders(List<string> published)
        {
            ...
            foreach (string folder in new List<string>(folders))
            {
                ...
                // `PublicAPI\V21\` is what V20's layout implies; `PublicAPI\` itself is there
