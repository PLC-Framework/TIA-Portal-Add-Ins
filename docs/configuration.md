...
`config.json` lives in `.plc-framework\` inside a TIA project and is what the Add-In reads.
This page is the authority on what it must contain, how it is validated, and how secrets
stay out of it.
...
[*The five locations, at a glance*](architecture.md): **`Program Files` is what the installer puts there,
