...
**Names TIA writes into a block are checked like every other name, and the configuration is what lets them pass** (2026-09-15). An OB's `Initial_Call` and `Remanence`, a GRAPH block's `INIT_SQ`, `ACK_EF`, `S_NO`, `S_MORE`, `S_ACTIVE` and `RT_DATA` are in the interface like anything else and nobody can rename them — so the template ships two interface rules, `interruption_parameter` and `graph_parameter`, assigned to the sections where TIA writes: an OB's `Input`, and a GRAPH block's `Input`, `Output` and `Static`. **Nothing in the Add-In knows any of those names.** Teaching the code to skip them would have put Siemens' vocabulary, and one GRAPH version of it, inside a binary that ships every few months; as a rule it is one line to adjust when a block type gains a parameter, and a project that does not use GRAPH never meets it at all.

**Both rules name the parameters outright and anchor at both ends.** A pattern by shape was written first — every underscore-separated segment starting with a capital — and it was too generous in the one place it mattered: a step called `S001_Init` satisfied it, so a step name that `sequence_variable` would have failed passed anyway. A list grows by one line when a GRAPH option brings another parameter, which the report names the first time it meets it; a shape quietly accepts names nobody wrote.

**The steps and transitions of a GRAPH block are not in those rules, and must not be.** TIA writes the members, but their names come from the GRAPH editor, where the engineer types them — which is exactly what `sequence_variable` and `transition_variable` exist to check. [`reference/S7-exports.md`](reference/S7-exports.md) shows how they sit in the file, and why the `Informative` attribute cannot be used to tell the two kinds apart.
...
- **And written beside the file, then swapped in**, through the same `SafeFile` everything under `repo\` uses. The whole file is rewritten on every call, so a write cut half way — a full disk, a machine that stops — would otherwise leave a truncated `.env`, and what is in there is a token nobody has a second copy of. A read-only `.env`, which is how one arrives off a copied profile, is replaced rather than refused.
