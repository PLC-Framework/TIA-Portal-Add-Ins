...
> **An Openness client has to locate the assemblies at run time**, from `HKLM\SOFTWARE\Siemens\Automation\Openness`: they are not in the GAC and cannot be shipped. A named entry is looked for first and its *folder* second, because V21 splits the object model across sixteen assemblies and the registry does not publish an entry for each. When it still fails, the window names what the loader said **and where it looked** — which keys existed, what they published, which folders were probed — rather than the process dying before it paints. The two failures worth telling apart are "the assemblies are not here" and "TIA refused the connection", the second of which usually means the Windows user is not in the **Siemens TIA Openness** group.
