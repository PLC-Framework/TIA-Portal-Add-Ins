...
> **A block that already exists is overwritten, and TIA says nothing.** `GenerateBlockOption` has only `None` and `KeepOnError` — no `Override` — so this had to be measured on the VM (2026-09-19): no refusal, no message. That is what *replace* means here, so nothing had to change — but it also means TIA will never stop a download, which is why the prompt above is the one safety this feature has.
