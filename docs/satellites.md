...
- **Nothing is read until you press it**, which is why it sits at the end of the PLC and software-unit row: choosing a scope and reading it are one decision. Opening the window counts what the scope holds — one pass over the tree, which is what fills the filter boxes — and waits; the caption under the project tree says so. A walk is minutes in V17–V20, and starting one because somebody opened a window, or looked at another unit, spends their time on a question they may not have asked.
