...
Picking a PLC and a unit **counts what is in there without reading any of it**, and offers the answer as tick boxes: `PlcStruct (91)`, `FB (31)`, `SCL (40)`, `LAD (25)`. Untick what this run is not about, and only what is left is mapped.
...
- **The choices come from the project, not from a list in the code.** `PlcBlock.ProgrammingLanguage` has twenty-nine values and a project uses four or five; a fixed list would be mostly empty rows. The counts also answer the question the operator actually has, which is not "does TIA have GRAPH" but "how much GRAPH is in *this* PLC".
- **A language filter never touches something that has no language.** A PLC data type and a tag table have none at all, so ticking `SCL` would otherwise delete all 91 UDTs — and 91 of the core's 249 sources *are* data types. Null and empty both mean "has none", and both pass; the first version of the rule only handled null and the second reading found it.
- **Everything starts ticked**, which is what this window did before there were any boxes, and `All` / `None` make narrowing to one kind two clicks rather than seven.
- **Nothing ticked is refused, not read as everything.** An empty filter means the whole scope — the safe reading of a decision nobody made — and an operator who has just cleared a panel has very much made one. The two would contradict each other in the one place it matters, so *Map project* switches off and says which panel is empty.
- **A filtered map records its filter and says so.** `project.json` carries `filter` beside the scope, and the window's heading reads *"Of what was ticked, not of the whole scope"*. Without that, a map covering only FBs says the project has no FCs — the silent hole this whole design exists to avoid.
- **The survey and the map walk the same code.** Two copies would drift, and the pair is worth nothing the moment the counts an operator ticked against and the map they then asked for cover different folders.
