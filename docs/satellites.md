...
- **The window reloads itself afterwards.** The comparison described the project as it was a moment ago, so it goes — but it used to stop there, at two empty trees and a line saying to map the project again, which is the same dead end the walk itself had one step earlier. What was just imported is the whole reason for looking. **What the import did survives that reload**: the summary and every refusal sit above whatever else is under the panels, because the reload rewrites both captions and the status line.
