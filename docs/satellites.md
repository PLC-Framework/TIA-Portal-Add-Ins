...
- **It opens as a small window in the corner of the screen, and grows once TIA has answered.** TIA Portal asks you to confirm that an application may access the project, and it asks while this is attaching — so a full-sized window, centred, lands on top of the box it asks in, and then both wait: TIA for your confirmation, this for TIA's answer. Small alone would not do it, because that dialog is centred too; the waiting window sits at the bottom right instead, and takes the middle of the screen once there is something to show.
...
- **It maps the PLC you picked, when you ask it to.** The Add-In sends the PLC that was right-clicked and `*` for the general program; the window offers that PLC's software units so the star is a starting point rather than a decision. **Load** walks blocks, PLC data types, tag tables and every folder, reads what each says about itself, writes `repo\project.json` and compares it with the core — and nothing reads the project until it is pressed.
...
- **One window per project.** A second launch on a project that already has one attaches, gives way and brings the first to the front; a window on another project opens beside it. The guard is taken once the attach has said which project this is, because before that nothing has — and it is keyed on the project's folder rather than on the TIA version, since a V20 and a V21 window on one folder would still write the same `repo\`. A project never saved takes none: there is no folder to write into.
...
A Load offers what it found as a grid: one row per kind of object, and on that row the languages found **inside that kind**. Untick what the next run is not about, and only what is left is mapped.

**The boxes come from the last Load of that scope, and nothing is walked to fill them.** They are written into `repo\project.json` by the walk itself, so a window opened on a project that has been mapped reads them off one file. A PLC never mapped has none — and no boxes is no filter, so the first Load of it reads the whole scope. That is the price of not walking the project to ask, and it is the one the maintainer chose.
...
- **Nothing ticked is refused, not read as everything.** An empty filter means the whole scope — the safe reading of a decision nobody made — and an operator who has just cleared a column has very much made one. So *Load* switches off and says so. **The same rule one row down**: a kind ticked with none of its languages would map nothing, and the row is named rather than silently skipped.
...
- **The counting happens inside the map's own walk, before the filter is asked** (2026-09-23). It used to be a walk of its own, run when the window opened and again on every change of scope — two passes over one tree for an answer the map's pass produces for nothing, and one of them spent on a question nobody had asked. It counts what the walk **visited** rather than what the filter kept, so a narrowed map still says what the whole scope holds; a filter built from what it let through could only ever be made smaller, each run forgetting what it had left out. **`Core` does the counting**, through a builder the adapter feeds one object at a time — how many of a kind there are is not a TIA question, and two adapters keeping their own tallies is exactly how they come to disagree.

### Holding the project against the core

The comparison reads `repo\project.json` back off disk, brings the project's copy of the core's graph up to date, and holds one against the other. It is the second half of what **Load** does. **Metadata only** — a version, a status and a family, never a line of code.
...
- **The core's graph is always copied into the project first**, as `repo\core.json`, and read out of the copy. A comparison against a repository that moves on describes a core nobody can point at afterwards; with a copy, both halves are on disk side by side for as long as the project keeps them. Configuration, repository, copy and catalogue are one call, `PlcCoreRefresh` — the copy has to happen before the catalogue is read out of it, and a caller that got that backwards would compare against whatever the last run left behind.
- **Only the graph, because the comparison reads nothing else.** Until 2026-09-22 a Load copied the whole core folder into `repo\core\` — 275 files, in every project, and on a remote core 277 requests against GitHub's sixty an hour without a token, so a public core could not be loaded at all without one. Now it is one file, and a second Load of a remote core that has not moved downloads nothing. A `repo\core\` left by an older version is removed on the next Load. The sources come down later, only for what is being imported — see *Downloading from the core*.
...
- **The caption says which core it was.** `Repository — PLC-Framework/code@main on api.github.com, commit abc1234 — 247 current nodes, 4 in the project, 3 at another version, 240 absent`, or the repository folder when the core is one on this machine. Until this, a local core and a remote one at a named commit gave the same line, and nothing on screen could date either. The answer comes from the refresh that just ran, never from the marker in `repo\` — a result must not describe a core other than the one it read — and a remote one whose graph came down says so too: *"(core.json updated)"*, left out entirely on the ordinary second run where nothing came down.
...
```
PLC  [A100              v]   Software unit  [*                     v]   [ Load ]
...
- **One Load, and it does the whole thing**: walk the project, bring the project's copy of the core up to date, compare the two. It was drawn as two buttons — one per side — and dropped before it was built: a core reload that did not re-walk TIA still had to re-read the map and compare, and one that did re-walk was the project reload under another name, which in V17–V20 costs one export per object. A comparison is only worth anything with both sides current.
- **Nothing is read until you press it**, which is why it sits at the end of the PLC and software-unit row: choosing a scope and reading it are one decision. Opening the window lists the PLCs, offers the filter boxes the last Load of that scope left in `repo\project.json`, and waits; the caption under the project tree says so. A walk is minutes in V17–V20, and starting one because somebody opened a window, or looked at another unit, spends their time on a question they may not have asked — which is why the count that used to fill those boxes is now part of the Load rather than a pass of its own.
- **Changing the PLC or the unit empties both panels.** What was on them came out of another scope, and a tree left standing under combo boxes that have moved is one nobody can date. The core tree goes too: every line on that side says what *this* project has of a node.
...
- **Inside a folder the blocks come first, then the data types, then the enumerations** — `.scl`, `.udt`, `.xlsx`, by name inside each — so `adt/queue` reads `_queue`, `queueInstanceAttributes`, `EQueueMethod`, `EQueueStatus`: a function and what it works on, rather than four names in alphabetical order.
...
- **The sources come down then, and only those.** Once it is settled what will be written — by the window below, or at once when the download meets nothing the project has — the sources of exactly what will be written — nothing for an entry left as it is — are copied from a local core or downloaded from a remote one into `repo\tmp\`, which the import reads from and clears when it is done. Where each goes in the project comes from `core.json`, so they land flat under their own names.
- **Always from the branch's head**, because the repository is the source of truth. A source the head no longer holds is refused by name rather than fetched from an older commit; **a single source that did not come down stops the whole import**, since whatever needed it would fail to build; and when the core has moved since **Load**, the import still goes ahead and says so — *compared at abc1234, imported from def5678* — because the plan was made from the older graph.
- **What you ticked is replaced; a dependency already at the core's version is left alone — to start with.** Asking for a block you already have is asking for it to be put back, which is how one that was edited or misfiled gets repaired.
- **Whenever the download would meet anything the project already has, you are asked first**, in a window listing every entry with a tick box:

  ```
  [ ] queueInstanceAttributes  v3.0  needed by _queue  v3.0 in PLC data types/core/adt/queue  leave as it is
  [x] _queue                   v3.0  ticked            v3.0 in Program blocks/core/adt/queue  overwrite
  1 of 2 ticked - 1 overwrite what the project has.
  ```

  Each line says why it is there, what the project has of it and where, what ticking it does — *import*, *overwrite*, *overwrite, and move into core/…* — and who else uses it. **TIA overwrites without a word**, so this list is the whole of the safety: nothing the project has is written without being on it. **`All` is how a block is downloaded a second time with everything it needs**, which the plan's own proposal — skip a dependency already at the core's version — never did. Leaving out a dependency the project does not have is allowed and shown in red, because whatever needs it will not build. A download that meets nothing the project has goes straight in.
- **Who else uses an entry is a floor, not a total**: dependency lists come from each block's `TITLE`, so the plant's own blocks declare nothing and are not counted — and the window says so above the list.
- **What you download ends up in its family's folder even when the project had it somewhere else.** TIA overwrites a block *where it is*, so one found elsewhere is exported, deleted and generated again in the right place. If it will not export — know-how protected, inconsistent — it is replaced where it stands and the report says why; if the new one will not generate, the old one is put back where it was; only if even that fails is it left as a file, named in the report, exactly as a move would leave it.
...
- **The window loads itself again afterwards, and it is the only thing it ever does unasked.** The comparison described the project as it was a moment ago, so it goes — but it used to stop there, at two empty trees and a line saying to map the project again, which is the same dead end the walk itself had one step earlier. What was just imported is the whole reason for looking, and this is not the window deciding to read a project: it is showing what a write did to one it was already showing. **What the import did survives it**: the summary and every refusal sit above whatever else is under the panels, because it rewrites both captions and the status line.
...
> **A block that already exists is overwritten, where it is, and TIA says nothing.** `GenerateBlockOption` has only `None` and `KeepOnError` — no `Override` — so this had to be measured on the VM (2026-09-19): no refusal, no message, and the new block left in the old one's folder whatever folder the call was given. Both halves shaped what is above: the list you are asked from, and the move.
...
- **The first row of `Constants` names the table and carries its `TITLE`** — the same JSON a block carries, read by the same reader — **and it is a constant like every other.** The first version read it for those two things and skipped it, so an imported table came out without the one constant that says where it came from; the next map then could not tell it from one of the plant's. There is nowhere else it could live: a `PlcTagTable` has a `Name` and nothing more, in either TIA version. Checked against `core.json` on all fifteen: the name is the node's `base`, and version, status and family agree with the graph.
...
- **A table already there is deleted and rebuilt, wherever it is.** An enumeration that has dropped a constant must drop it here too; creating over the top would leave the retired one behind with nothing saying so. It is found in the whole tree rather than only in its family's folder — a table of that name elsewhere made the create fail — and it is exported first, so it can be put back if the new one cannot be made.
...
- **Which leaves a moment where the object is only a file, and that is what the report is built around.** A move whose import fails keeps its export and names the path. The export is written in `.plc-framework\repo\stranded\` from the start, and nothing ever clears that folder — an object nobody can find again is the one outcome this must not produce. **The window says what is waiting there as soon as it opens**, and on every run after, until somebody imports it back in TIA Portal or deletes it — the report that named it is gone the moment the window reloads. It is a fact about the project's folder rather than about TIA, so it needs no Load to be read.
